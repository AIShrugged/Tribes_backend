<?php

namespace App\Services\Agent\Tools;

use App\Enums\MeetingTaskStatus;
use App\Jobs\RegenerateFollowupJob;
use App\Models\AgentProfile;
use App\Models\AgentTask;
use App\Models\CalendarEvent;
use App\Models\ChannelMessage;
use App\Models\Followup;
use App\Models\Issue;
use App\Models\Profile;
use App\Models\User;
use App\Services\JsonSchemaValidationService;
use App\Services\TenantScopeValidator;
use Illuminate\Database\Eloquent\Builder;
use Illuminate\Support\Facades\Auth;
use Illuminate\Support\Facades\Gate;
use Illuminate\Validation\ValidationException;

class CreateEntityTool extends AbstractAgentTool
{
    private const SOURCEABLE_MAP = [
        'calendar_event' => CalendarEvent::class,
        'channel_message' => ChannelMessage::class,
        'chat_message' => ChannelMessage::class,
        'telegram_chat_message' => ChannelMessage::class,
    ];

    public function __construct(
        private readonly ?User $user = null,
        private readonly ?TenantScopeValidator $tenantScopeValidator = null,
        private readonly ?JsonSchemaValidationService $schemaValidation = null,
        private readonly ?int $defaultOrganizationId = null,
        private readonly ?int $defaultTeamId = null,
    ) {
        parent::__construct();
    }

    public function getName(): string
    {
        return 'create_entity';
    }

    public function getDescription(): string
    {
        return 'Create a new entity: a user-facing issue, a background agent task, or regenerate a followup report. '
            .'Use entity="issue" for tasks tracked by people, entity="agent_task" to delegate work to a background agent, '
            .'entity="followup" to regenerate a stale meeting followup.';
    }

    public function getParameters(): array
    {
        $validStatuses = array_map(fn ($s) => $s->value, MeetingTaskStatus::cases());

        return [
            'type' => 'object',
            'required' => ['entity', 'data'],
            'properties' => [
                'entity' => [
                    'type' => 'string',
                    'enum' => ['issue', 'agent_task', 'followup'],
                    'description' => 'Type of entity to create.',
                ],
                'data' => [
                    'type' => 'object',
                    'description' => 'Entity-specific fields.',
                    'properties' => [
                        // issue fields
                        'name' => ['type' => 'string', 'description' => 'Short name. Required for issue and agent_task.'],
                        'description' => ['type' => 'string', 'description' => 'Detailed description (issue only).'],
                        'type' => [
                            'type' => 'string',
                            'enum' => Issue::TYPES,
                            'description' => 'Issue type (issue only).',
                        ],
                        'status' => [
                            'type' => 'string',
                            'enum' => $validStatuses,
                            'description' => 'Initial issue status (issue only). Defaults to open.',
                        ],
                        'organization_id' => ['type' => 'integer'],
                        'team_id' => ['type' => 'integer'],
                        'assignee_name' => ['type' => 'string'],
                        'assignee_id' => ['type' => 'integer'],
                        'due_date' => ['type' => 'string', 'description' => 'YYYY-MM-DD (issue only).'],
                        'sourceable_type' => [
                            'type' => 'string',
                            'enum' => ['calendar_event', 'channel_message', 'chat_message', 'telegram_chat_message'],
                            'description' => 'Source entity type (issue only).',
                        ],
                        'sourceable_id' => ['type' => 'integer', 'description' => 'Source entity id (issue only).'],
                        // agent_task fields
                        'prompt' => ['type' => 'string', 'description' => 'Instruction for the background agent (agent_task only).'],
                        'agent_profile_id' => ['type' => 'integer', 'description' => 'Optional agent profile id (agent_task only).'],
                        'schedule_type' => [
                            'type' => 'string',
                            'enum' => ['one_off', 'interval'],
                            'description' => 'Scheduling mode (agent_task only). Defaults to one_off.',
                        ],
                        'interval_seconds' => ['type' => 'integer', 'description' => 'Required when schedule_type=interval.'],
                        'next_run_at' => ['type' => 'string', 'description' => 'Optional ISO-8601 first run datetime (agent_task only).'],
                        'execution_mode' => [
                            'type' => 'string',
                            'enum' => ['inline', 'isolated', 'paperclip'],
                            'description' => 'Optional execution mode (agent_task only).',
                        ],
                        'sandbox_profile' => ['type' => 'string'],
                        'agent_task_type' => ['type' => 'string'],
                        'output_mode' => ['type' => 'string'],
                        'allowed_tools' => ['type' => 'array', 'items' => ['type' => 'string']],
                        'allowed_outbound_hosts' => ['type' => 'array', 'items' => ['type' => 'string']],
                        'input_payload' => ['type' => 'object', 'additionalProperties' => true],
                        'enabled' => ['type' => 'boolean'],
                        'max_attempts' => ['type' => 'integer'],
                        'metadata' => ['type' => 'object', 'additionalProperties' => true],
                        // followup fields
                        'calendar_event_id' => ['type' => 'integer', 'description' => 'Calendar event to regenerate followup for (followup only).'],
                    ],
                ],
            ],
        ];
    }

    public function execute(?array $parameters): mixed
    {
        $parameters = $parameters ?? [];
        $entity = $parameters['entity'] ?? '';
        $data = $parameters['data'] ?? [];

        if (! is_array($data)) {
            $data = [];
        }

        return match ($entity) {
            'issue' => $this->createIssue($data),
            'agent_task' => $this->createAgentTask($data),
            'followup' => $this->createFollowup($data),
            default => ['success' => false, 'error' => "Unknown entity: {$entity}. Must be one of: issue, agent_task, followup"],
        };
    }

    private function createIssue(array $data): array
    {
        $name = trim((string) ($data['name'] ?? ''));
        if ($name === '') {
            return ['success' => false, 'error' => 'data.name is required for issue'];
        }

        $type = trim((string) ($data['type'] ?? ''));
        if (! in_array($type, Issue::TYPES, true)) {
            return ['success' => false, 'error' => 'data.type must be one of: '.implode(', ', Issue::TYPES)];
        }

        $status = $data['status'] ?? MeetingTaskStatus::OPEN->value;
        $validStatuses = array_map(fn ($s) => $s->value, MeetingTaskStatus::cases());
        if (! in_array($status, $validStatuses, true)) {
            return ['success' => false, 'error' => 'data.status must be one of: '.implode(', ', $validStatuses)];
        }

        $sourceableType = null;
        $sourceableId = null;
        $sourceTypeKey = $data['sourceable_type'] ?? $data['taskable_type'] ?? null;

        if ($sourceTypeKey !== null) {
            if (! isset(self::SOURCEABLE_MAP[$sourceTypeKey])) {
                return ['success' => false, 'error' => 'Unknown sourceable_type: '.$sourceTypeKey];
            }
            $sourceableType = self::SOURCEABLE_MAP[$sourceTypeKey];
            $sourceableId = $data['sourceable_id'] ?? $data['taskable_id'] ?? null;

            if (! $sourceableId) {
                return ['success' => false, 'error' => 'data.sourceable_id is required when sourceable_type is provided'];
            }
        }

        $user = $this->resolveCurrentUser();
        if (! $user) {
            return ['success' => false, 'error' => 'Authenticated user context is required'];
        }

        $organizationId = isset($data['organization_id'])
            ? (int) $data['organization_id']
            : $this->defaultOrganizationId;
        $teamId = array_key_exists('team_id', $data)
            ? ($data['team_id'] !== null ? (int) $data['team_id'] : null)
            : $this->defaultTeamId;

        try {
            $this->tenantScopeValidator()->assertScopeIsValid($user, $organizationId, $teamId, allowUnbound: false);

            if ($teamId === null && ! $user->isOrganizationMember((int) $organizationId)) {
                throw ValidationException::withMessages([
                    'organization_id' => ['Only organization members can create organization-level issues without a team scope.'],
                ]);
            }

            $assigneeId = $data['assignee_id'] ?? $this->resolveAssigneeIdFromProfile($data);
            $this->assertAssigneeIsVisible($user, $assigneeId !== null ? (int) $assigneeId : null);
        } catch (ValidationException $exception) {
            return [
                'success' => false,
                'error' => collect($exception->errors())->flatten()->first() ?? 'Validation failed',
            ];
        }

        $issue = Issue::create([
            'user_id' => $user->id,
            'organization_id' => $organizationId,
            'team_id' => $teamId,
            'sourceable_type' => $sourceableType,
            'sourceable_id' => $sourceableId,
            'assignee_id' => $assigneeId ?? null,
            'name' => $name,
            'description' => $data['description'] ?? null,
            'type' => $type,
            'assignee_name' => $data['assignee_name'] ?? null,
            'due_date' => $data['due_date'] ?? null,
            'status' => $status,
        ]);

        return [
            'success' => true,
            'issue' => [
                'id' => $issue->id,
                'name' => $issue->name,
                'description' => $issue->description,
                'type' => $issue->type,
                'status' => $issue->status,
                'organization_id' => $issue->organization_id,
                'team_id' => $issue->team_id,
                'assignee_name' => $issue->assignee_name,
                'assignee_id' => $issue->assignee_id,
                'due_date' => $issue->due_date?->toDateString(),
                'sourceable_type' => $issue->sourceable_type,
                'sourceable_id' => $issue->sourceable_id,
            ],
        ];
    }

    private function createAgentTask(array $data): array
    {
        $user = $this->resolveCurrentUser();
        if (! $user) {
            return ['success' => false, 'error' => 'Authenticated user context is required'];
        }

        $name = trim((string) ($data['name'] ?? ''));
        if ($name === '') {
            return ['success' => false, 'error' => 'data.name is required for agent_task'];
        }

        $prompt = trim((string) ($data['prompt'] ?? ''));
        if ($prompt === '') {
            return ['success' => false, 'error' => 'data.prompt is required for agent_task'];
        }

        $organizationId = isset($data['organization_id'])
            ? (int) $data['organization_id']
            : $this->defaultOrganizationId;
        $teamId = array_key_exists('team_id', $data)
            ? ($data['team_id'] !== null ? (int) $data['team_id'] : null)
            : $this->defaultTeamId;
        $scheduleType = (string) ($data['schedule_type'] ?? 'one_off');
        $intervalSeconds = isset($data['interval_seconds']) ? (int) $data['interval_seconds'] : null;

        if (! in_array($scheduleType, ['one_off', 'interval'], true)) {
            return ['success' => false, 'error' => 'data.schedule_type must be one of: one_off, interval'];
        }

        if ($scheduleType === 'interval' && ($intervalSeconds === null || $intervalSeconds < 1)) {
            return ['success' => false, 'error' => 'data.interval_seconds is required for interval tasks'];
        }

        $profile = null;
        if (! empty($data['agent_profile_id'])) {
            $profile = AgentProfile::query()->find($data['agent_profile_id']);
            if (! $profile) {
                return ['success' => false, 'error' => 'data.agent_profile_id was not found'];
            }
        }

        $inputPayload = $data['input_payload'] ?? [];
        if ($inputPayload !== null && ! is_array($inputPayload)) {
            return ['success' => false, 'error' => 'data.input_payload must be an object'];
        }

        try {
            $this->tenantScopeValidator()->assertScopeIsValid($user, $organizationId, $teamId, allowUnbound: false);

            $this->schemaValidation()->validatePayload(
                is_array($inputPayload) ? $inputPayload : [],
                $profile?->task_payload_schema,
            );
        } catch (ValidationException $exception) {
            return [
                'success' => false,
                'error' => collect($exception->errors())->flatten()->first() ?? 'Validation failed',
            ];
        }

        $task = AgentTask::create([
            'user_id' => $user->id,
            'organization_id' => $organizationId,
            'team_id' => $teamId,
            'agent_profile_id' => $profile?->id,
            'name' => $name,
            'prompt' => $prompt,
            'schedule_type' => $scheduleType,
            'execution_mode' => $data['execution_mode'] ?? null,
            'sandbox_profile' => $data['sandbox_profile'] ?? null,
            'interval_seconds' => $scheduleType === 'interval' ? $intervalSeconds : null,
            'agent_task_type' => $data['agent_task_type'] ?? 'background',
            'output_mode' => $data['output_mode'] ?? 'plain',
            'allowed_tools' => $data['allowed_tools'] ?? null,
            'allowed_outbound_hosts' => $data['allowed_outbound_hosts'] ?? null,
            'input_payload' => $inputPayload,
            'next_run_at' => $data['next_run_at'] ?? now(),
            'enabled' => array_key_exists('enabled', $data) ? (bool) $data['enabled'] : true,
            'max_attempts' => isset($data['max_attempts']) ? (int) $data['max_attempts'] : 3,
            'metadata' => [
                ...($data['metadata'] ?? []),
                'profile_metadata' => is_array($profile?->metadata) ? $profile->metadata : [],
            ],
        ]);

        return [
            'success' => true,
            'agent_task' => [
                'id' => $task->id,
                'name' => $task->name,
                'organization_id' => $task->organization_id,
                'team_id' => $task->team_id,
                'agent_profile_id' => $task->agent_profile_id,
                'schedule_type' => $task->schedule_type?->value ?? $task->schedule_type,
                'interval_seconds' => $task->interval_seconds,
                'execution_mode' => $task->execution_mode?->value ?? $task->execution_mode,
                'next_run_at' => $task->next_run_at?->toIso8601String(),
                'enabled' => (bool) $task->enabled,
            ],
        ];
    }

    private function createFollowup(array $data): array
    {
        $calendarEventId = (int) ($data['calendar_event_id'] ?? 0);

        if ($calendarEventId < 1) {
            return ['success' => false, 'error' => 'data.calendar_event_id is required for followup'];
        }

        $user = $this->resolveCurrentUser();
        if (! $user) {
            return ['success' => false, 'error' => 'Authenticated user context is required'];
        }

        $followup = Followup::with(['calendarEvent', 'team', 'user', 'methodology'])
            ->where('calendar_event_id', $calendarEventId)
            ->owned($user->id)
            ->latest('id')
            ->first();

        if (! $followup) {
            return ['success' => false, 'error' => 'Followup not found for the given calendar event or access denied'];
        }

        if (! Gate::forUser($user)->allows('view', $followup)) {
            return ['success' => false, 'error' => 'Followup not found for the given calendar event or access denied'];
        }

        RegenerateFollowupJob::dispatch($followup->calendar_event_id, $user->id);

        return [
            'success' => true,
            'old_followup_id' => $followup->id,
            'calendar_event_id' => $followup->calendar_event_id,
            'status' => 'in_progress',
            'message' => 'Followup regeneration started',
        ];
    }

    private function resolveCurrentUser(): ?User
    {
        $user = $this->user ?? Auth::user();

        return $user instanceof User ? $user : null;
    }

    private function resolveAssigneeIdFromProfile(array $data): ?int
    {
        $profileId = $data['profile_id'] ?? null;

        if (! $profileId) {
            return null;
        }

        return Profile::query()->find($profileId)?->user_id;
    }

    private function assertAssigneeIsVisible(User $user, ?int $assigneeId): void
    {
        if ($assigneeId === null) {
            return;
        }

        $organizationIds = $user->organizations()->pluck('organizations.id');
        $teamIds = $user->teams()->pluck('teams.id');

        $query = User::query()->whereKey($assigneeId);
        $query->where(function (Builder $builder) use ($organizationIds, $teamIds, $user): void {
            $builder->whereKey($user->id);

            if ($organizationIds->isNotEmpty()) {
                $builder->orWhereHas('organizations', function (Builder $relation) use ($organizationIds): void {
                    $relation->whereIn('organizations.id', $organizationIds);
                });
            }

            if ($teamIds->isNotEmpty()) {
                $builder->orWhereHas('teams', function (Builder $relation) use ($teamIds): void {
                    $relation->whereIn('teams.id', $teamIds);
                });
            }
        });

        if (! $query->exists()) {
            throw ValidationException::withMessages([
                'assignee_id' => ['The selected assignee is not available to the current user.'],
            ]);
        }
    }

    private function tenantScopeValidator(): TenantScopeValidator
    {
        return $this->tenantScopeValidator ?? app(TenantScopeValidator::class);
    }

    private function schemaValidation(): JsonSchemaValidationService
    {
        return $this->schemaValidation ?? app(JsonSchemaValidationService::class);
    }
}
