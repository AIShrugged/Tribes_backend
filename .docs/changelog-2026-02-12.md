...
## Ветка: TBS-0000-insight-telegram-enrichment

**Коммит:** `637b03c` (17:55)
**Залито в:** dev

---

## Telegram-чат обогащает профили сотрудников

Система теперь анализирует переписку пользователя с AI-ботом и автоматически пополняет его профиль — без участия HR, без ручных действий.

### Что изменилось

**Новый сервис `InsightTelegramService`** — берёт историю сообщений из `telegram_chat_messages`, отправляет в AI, записывает результат в Insight.

**Новая команда** `php artisan insight:process-telegram [--user=ID]` — запускается каждые **4 часа** по расписанию.

**Расширен `InsightPromptBuilder`** — добавлен промпт для анализа чата (один пользователь, неформальный контекст). Факты извлекаются только из слов пользователя; сообщения бота используются как контекст.

**Исправлен `AppServiceProvider`** — прокси теперь отключается через `PROXY_ENABLED=false`.

### Что записывается в профиль

**Долгосрочная память** (`insight_items` → `insight_profiles`) — поведенческие факты по 6 категориям: стиль общения, паттерны работы, сильные стороны, зоны роста, цели и мотивация, психологический профиль. Каждый факт версионируется — история не теряется.

**Краткосрочная память** (`insight_short_term`) — текущий контекст с TTL: над чем работает (30 дней), какие решения принял (30 дней), эмоциональное состояние (7 дней).

**Отношения между участниками** (`insight_relationships`) — попарная динамика на встречах: тип (collaborative / conflicting / hierarchical / neutral), наблюдения, позитивные и негативные паттерны. 

### Условие запуска

Анализ происходит если: ≥ 10 сообщений от пользователя **или** прошли сутки с последнего запуска. Обработанные сообщения не перечитываются повторно.

### Ограничение

Работает только для пользователей с привязанным аккаунтом (`telegram_users.user_id IS NOT NULL`).
